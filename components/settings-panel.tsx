...
export function SettingsPanel({
  ...
  useEffect(() => {
    const storedSettings = localStorage.getItem("yesterday-core:settings");
    const storedTheme = (localStorage.getItem("yesterday-core:theme") as Theme | null) ?? "system";
    queueMicrotask(() => {
    ...
    void fetch("/api/settings")
      ...
      .then((body: { settings?: Settings } | null) => {
        if (body?.settings) {
          ...
          localStorage.setItem("yesterday-core:settings", JSON.stringify(body.settings));
        }
      });
  }, []);
  ...
  function update(next: Settings) {
    setSettings(next);
    setSaved(false);
    localStorage.setItem("yesterday-core:settings", JSON.stringify(next));
    void fetch("/api/settings", {
      method: "POST",
  ...
  function updateTheme(next: Theme) {
    setTheme(next);
    localStorage.setItem("yesterday-core:theme", next);
    applyTheme(next);
  }
  ...
  async function regenerate() {
    ...
    setRegenerateMessage(response.ok ? "어제 브리핑을 다시 생성했습니다." : "재생성에 실패했습니다.");
    ...
    <div className="settings-sections">
      ...
      <section className="settings-group">
        ...
        <button className="secondary-button" type="button" onClick={regenerate} disabled={regenerating}>
          {regenerating ? "다시 생성 중…" : "어제 뉴스 다시 생성"}
        </button>
